...
@section('content')
    <div class="container mt-5">
        ...
        <form action="{{ route('mutasi_lokasi.update', $mutasiLokasi->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
                ...
                <select class="form-control" id="id_lokasi" name="id_lokasi" required>
                    @foreach($lokasi as $lokasiItem)
                        <option value="{{ $lokasiItem->id }}" {{ $lokasiItem->id == $mutasiLokasi->id_lokasi ? 'selected' : '' }}>
            ...
            <div class="mb-3">
                ...
                <select class="form-control" id="id_pengadaan" name="id_pengadaan" required>
                    @foreach($pengadaan as $pengadaanItem)
                        <option value="{{ $pengadaanItem->id }}" {{ $pengadaanItem->id == $mutasiLokasi->id_pengadaan ? 'selected' : '' }}>
